...
class SimpleTTSRequest(BaseModel):
    """Request body for the /tts convenience endpoint.

    A thin, single-shot wrapper over /v1/text-to-speech: JSON in, complete
    audio out. No streaming, no voice cloning.
    """
    text: str = Field(..., min_length=1, max_length=5000, description="Text to synthesize")
    voice: Optional[str] = Field(
        None,
        description="Voice ID (e.g. 'kn_female') or speaker ID (e.g. 'Kannada (Female)'). "
                    "Falls back to the server's DEFAULT_VOICE when omitted.",
    )
    format: Literal["wav", "pcm"] = Field(
        "wav",
        description="'wav' returns a playable RIFF file; 'pcm' returns raw 24kHz mono PCM16.",
    )
