...
except ImportError:  # decoding falls back to torchaudio
    sf = None
...
def _decode_audio(buffer: BytesIO) -> Tuple[torch.Tensor, int]:
    """Decode an audio file into a (channels, frames) tensor and its sample rate.

    soundfile is tried first. torchaudio 2.10 delegates decoding to TorchCodec,
    a separate package pinned to the torch build, so torchaudio.load raises
    ImportError in images that do not ship it. libsndfile covers WAV, FLAC, OGG
    and MP3, which is every format a reference recording is likely to arrive in.
    """
    if sf is not None:
        try:
            data, sample_rate = sf.read(buffer, dtype="float32", always_2d=True)
            # soundfile returns (frames, channels); torchaudio uses (channels, frames)
            return torch.from_numpy(data.T.copy()), sample_rate
        except Exception:
            logger.debug("soundfile could not decode this audio; trying torchaudio",
                         exc_info=True)
            buffer.seek(0)
...
def load_audio_from_bytes(
    ...
    try:
        ...
        waveform, sample_rate = _decode_audio(audio_buffer)
